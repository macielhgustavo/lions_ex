...
export class ProductService {
  ...
  private validate(data: Omit<IProduct, 'id'>): void {
    if (data.name.trim().length < 3) throw new AppError('Nome deve ter pelo menos 3 caracteres', 400);
    if (Number.isNaN(data.price) || data.price < 0) throw new AppError('Preço inválido', 400);
  }
}
